"""
Teste 2 — Tese RMC (Reserva de Margem Consignável).
...
COM_BASE = os.path.join(ASSETS, '__BASE_com-escritorio__modelo-rmc-Tiago-AL-BMG.docx')
COM_TEMPLATE = os.path.join(ASSETS, 'template_rmc__com-escritorio.docx')
SEM_TEMPLATE = os.path.join(ASSETS, 'template_rmc__sem-escritorio.docx')
...
mapa_rmc = {
    ...
    # Contexto fático — banco + contrato (string longa)
    'RESERVA DE MARGEM CONSIGNÁVEL (RMC) pelo BANCO BMG S.A, sob o contrato nº 15076520':
        'RESERVA DE MARGEM CONSIGNÁVEL (RMC) pelo {{NOME_BANCO_CONTRATO}}, sob o contrato nº {{CONTRATO_NUMEROS}}',

    # Cabeçalho data
    'Arapiraca/AL, 9 de dezembro de 2025': '{{CIDADE_ASSINATURA}}/{{UF_ASSINATURA}}, {{DATA_EXTENSO}}',
    ...
    'OAB/AL  20906A': '{{ADVOGADO_OAB_UF}}',  # corpo
    'TIAGO DE AZEVEDO LIMA': '{{ADVOGADO_NOME_MAIUSCULO}}',  # assinatura final
    'OAB/AL 20906A': '{{ADVOGADO_OAB_UF}}',  # rodapé

    # Cancelamento
    'O Cancelamento definitivo do Contrato nº 15076520':
        'O Cancelamento definitivo do(s) Contrato(s) nº {{CONTRATO_NUMEROS}}',

    # Dano moral RMC = R$ 10.000,00 (dez mil reais) — diferente do NC (R$ 15k)
...
print('=== ETAPA 1A: Template RMC COM escritório ===')
rel = substituir_em_docx(COM_BASE, mapa_rmc, COM_TEMPLATE)
...
print('\n=== ETAPA 1B: Template RMC SEM escritório (clone COM + timbrado neutro) ===')
...
# Fontes vêm do BASE formatado manualmente (Cambria/Segoe UI) — nada a fazer aqui.

# Teste COM
TESTE_COM = os.path.join(BASE, '_TESTE_rmc__com-escritorio.docx')
cliente_com = {
    **cliente_F,
...
print('\n=== ETAPA 2A: Gerando teste RMC COM ===')
rel = substituir_em_docx(COM_TEMPLATE, cliente_com, TESTE_COM)
print(f'  Substituições: {rel["total_substituicoes"]}')

# Teste SEM
TESTE_SEM = os.path.join(BASE, '_TESTE_rmc__sem-escritorio.docx')
cliente_sem = {
    **cliente_F,
...
print('\n=== ETAPA 2B: Gerando teste RMC SEM ===')
